...
sensor_config["temperature"] = 15.3
sensor_config["moisture"] = 40
...
#PREPARING MESSAGE FOR DEVICE
    ...
    if(sensor_config["state"] == "OFF"): 
        ...
        sensor_config["temperature"] = random.randrange(10, 50)    
    
    sensor_message = ('{ "date":"'+str(time_string)+'",'
        +'"agent_id":"'+str(sensor_config["agent_id"])+'",'
        +'"temperature":'+str(sensor_config["temperature"])+','
        +'"moisture":'+str(sensor_config["moisture"])+','
...
def reset_values_sensors():
    sensor_config["agent_id"] = "AGENTX"
    sensor_config["temperature"] = 0
    sensor_config["moisture"] = 0
